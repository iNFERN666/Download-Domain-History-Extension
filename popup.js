...
function downloadFile() {
    ...
    chrome.history.search({text: '', maxResults: 1000000, startTime: 0}, function(historyItems) {
        // Extract the unique domains from the history
        const domains = new Set();
        for (let i = 0; i < historyItems.length; i++) {
            const url = new URL(historyItems[i].url);
            const hostname = url.hostname.split('.');
            domains.add(hostname[hostname.length - 2] + "." + hostname[hostname.length - 1]);
        }
        // Convert the set to an array and join it into a string
        const domainString = Array.from(domains).join('\n');
